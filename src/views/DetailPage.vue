<template>
  <ion-page>
    <ion-header translucent>
      <ion-toolbar>
        <ion-title class="ion-text-center"> Ajouter un etudiant</ion-title>
      </ion-toolbar>
    </ion-header>
    ...
    <ion-content :fullscreen="true">
      <ion-item>
        ...
        <ion-select v-model="civilite" required>
          <ion-select-option
      ...
      <ion-list>
        <ion-radio-group value="specialité" v-model="specialite">
          ...
          <ion-text class="ion-text-center">
            <h2>Ceci est votre affichage:<br /></h2>
            <h4>
              {{ civilite }} {{ nom }} {{ prenom }} Vous avez comme specialité
              la suivante : {{ specialite }}<br />
              Comme matières vous avez selectioné ceci:
              <!-- {{matieres.filter(item => item.checked).map(item => item.label)}} -->
              <div
                v-for="(item, index) in matieres
